...
sampl_offset = 0; % Aligns the sampling between the synthesis and analysis band
...
[~,sync_point] = max(abs(P).*abs(PP(length(h1)/2+1:end-length(h1)/2)));

%% DEMODULATOR START:
RxStage1 = RxSignal(sync_point + sampl_offset: sync_point + sampl_offset + Ns3+Ncp3+Nzp3+Ncs3+Nzp3-1);
...
signal_for_FEQ = RxStage6a(1:Lpreamble*M1+l1-1,:);
[all_channel, Channel_estimate, channel_estimate]  = tmp_channel_estimation(signal_for_FEQ, OutSignal,M1, M1, 2.5*M1, 2.5*M1, 0, 0, 0, e1);
