        ...
        while (getline(is_file_content, line)) {
            ...
            if (getline(is_line, key, CONFIG_DELIM)) {
                ...
                if (getline(is_line >> std::ws, value)) {
                    if (key.find("usrp_master_clock") != string::npos) {
                    ...
                    }  else if (key.find("usrp_tx_rate") != string::npos) {
                        size_t pos_start = 0; size_t pos = 0; size_t pos_end;
                        vector<double> tmp;
                        string tmpstr;
                        while ((pos!=string::npos)) {
                        ...
                        config->usrp_tx_rate[identified_usrp_systems] = tmp;
                        ...
                        config->usrp_number_of_samples.push_back(atoi(value.c_str()));
                    } else if (key.find("usrp_lo_off") != string::npos) {
                        config->usrp_lo_off.push_back(atof(value.c_str()));
...
void initvalues(UsrpConfig * config) {
    ...
    config->usrp_lo_off.push_back(0.0);
